...
import { isSpontaneousMessagesEnabled, setSpontaneousMessagesEnabled, requestNotificationPermission } from './lib/spontaneousMessages';
...
export default function App() {
  ...
  const { messagesMap, isTypingMap, sendMessage, initChat, setMessagesForContact, hasLoadedMessages, triggerSpontaneousTest } = useGeminiChat(activeContactConfig);
  ...
  const [spontaneousEnabled, setSpontaneousEnabled] = useState(() => isSpontaneousMessagesEnabled());
            ...
            return (
                <div className="flex flex-col h-full bg-[var(--color-wa-bg)]">
                    ...
                    <div className="flex-1 overflow-y-auto bg-[var(--color-wa-bg)] custom-scrollbar">
                        ...
                        <div className="px-6 py-4 flex justify-between items-center bg-[var(--color-wa-bg)] border-b border-[var(--color-wa-border)] cursor-pointer" onClick={async () => {
                            const perm = await requestNotificationPermission();
                            showToast(perm === 'granted' ? 'Meldingen ingeschakeld!' : 'Status: ' + perm);
                        }}>
                            <div>
                                <div className="text-[15px] text-[#e9edef]">Systeem- & browsermeldingen</div>
                                <div className="text-[13px] text-[#8696a0]">Krijg een pop-up melding als Gerda uit zichzelf appt</div>
                            </div>
                            <span className="text-xs px-2.5 py-1 bg-[#00a884]/20 text-[#00a884] rounded-full font-medium">Toestaan</span>
                        </div>
                        <div className="px-6 py-4 flex justify-between items-center bg-[var(--color-wa-bg)] border-b border-[var(--color-wa-border)]">
            ...
            return (
                <div className="flex flex-col h-full bg-[#0b141a]">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar pt-2">
                        ...
                        <div className="px-6 py-3.5 flex justify-between items-center cursor-pointer" onClick={() => {
                           const v = !spontaneousEnabled;
                           setSpontaneousEnabled(v);
                           setSpontaneousMessagesEnabled(v);
                           if (v) requestNotificationPermission();
                           showToast(v ? 'Spontane berichten aan (9:00 - 20:00, ~3x per dag)' : 'Spontane berichten uitgeschakeld');
                        }}>
                           <div>
                             <div className="text-[#e9edef] text-[17px]">Spontane berichten (9:00 - 20:00)</div>
                             <div className="text-[#8696a0] text-[14px] pr-8">Gerda stuurt willekeurig ~3 keer per dag zelf een berichtje met WhatsApp-meldingen</div>
                           </div>
                           <input type="checkbox" className="w-5 h-5 accent-[#00a884]" checked={spontaneousEnabled} readOnly />
                        </div>
                        <div className="px-6 py-2 pb-4">
                           <button
                             onClick={async () => {
                               showToast('Gerda stuurt nu een spontaan bericht...');
                               try {
                                 await triggerSpontaneousTest();
                                 showToast('Bericht & melding ontvangen!');
                               } catch (e) {
                                 showToast('Fout bij sturen bericht');
                               }
                             }}
                             className="px-4 py-2 bg-[#00a884]/20 text-[#00a884] rounded-lg text-sm font-medium hover:bg-[#00a884]/30 transition-colors flex items-center gap-2"
                           >
                             <span>🔔</span> Test spontaan bericht & melding nu
                           </button>
                        </div>
